// Standalone seed script — run directly with bun
// Usage: DATABASE_URL="..." bun scripts/seed-supabase.ts
...
// Officer data
const OFFICERS = [
  { name: "Reiner Nuevas", position: "PRESIDENT" as const, order: 1, email: "rnuevas.k12042427@umak.edu.ph" },
  { name: "Jennylyn S. Pelovello", position: "VICE_PRESIDENT_INTERNAL" as const, order: 2, email: "jpelovello.a12344479@umak.edu.ph" },
  { name: "Alexander C. Cerilla", position: "VICE_PRESIDENT_EXTERNAL" as const, order: 3, email: "acerilla.k12151773@umak.edu.ph" },
  { name: "Nicolas Nikolai L. Clavo", position: "SECRETARY" as const, order: 4, email: "nclavo.k12255841@umak.edu.ph" },
  { name: "John Paulo C. Dingle", position: "TREASURER" as const, order: 5, email: "jdingle.a12343741@umak.edu.ph" },
  { name: "Marc Aeron B. Cruz", position: "AUDITOR" as const, order: 6, email: "mcruz.k12148911@umak.edu.ph" },
  { name: "Daniela M. Martinez", position: "PUBLIC_RELATION_OFFICER" as const, order: 7, email: "dmartinez.a12343893@umak.edu.ph" },
];
...
async function main() {
  ...
  let users = 0, offices = 0, officers = 0, saProfiles = 0;
  ...
  users++;
  console.log("  ✅ superadmin@umak.edu.ph");
  ...
  const advP = await db.officerProfile.findUnique({ where: { userId: adviser.id } });
  if (!advP) await db.officerProfile.create({ data: { userId: adviser.id, position: "ADVISER", orderIndex: 0 } });
  users++; officers++;
  console.log("  ✅ adviser@umak.edu.ph");
  ...
  for (const o of OFFICERS) {
    const user = await db.user.upsert({
      where: { email: o.email },
      update: { password: "UMAKSAS@Officer_2026", firstName: o.name, lastName: "", role: "OFFICER", isActive: true },
      create: { email: o.email, password: "UMAKSAS@Officer_2026", firstName: o.name, lastName: "", role: "OFFICER", isActive: true },
    });
    const p = await db.officerProfile.findUnique({ where: { userId: user.id } });
    if (!p) await db.officerProfile.create({ data: { userId: user.id, position: o.position, orderIndex: o.order } });
    users++; officers++;
    console.log(`  ✅ ${o.email} (${o.position})`);
  }
  ...
  // Link HRMO office
  const hrmoOid = officeIdByCode.get("HRMO");
  if (hrmoOid) await db.office.update({ where: { id: hrmoOid }, data: { headUserId: hrmo.id } });

  // 6. OFFICE SUPERVISORS
  console.log("\n📋 Creating Office Supervisors...");
  for (const [, info] of officeMap) {
    const code = info.code.toUpperCase();
    if (code === "HRMO" || !info.email) continue;
    try {
  ...
  console.log(`  ✅ ${saProfiles} SA profiles`);

  console.log("\n" + "=".repeat(40));
  console.log("🎉 SEED COMPLETE!");
  console.log(`   Users: ${users} | Offices: ${offices} | Officers: ${officers} | SAs: ${saProfiles}`);
  console.log("\n🔑 Login Credentials:");
